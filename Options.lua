...
local function BuildShelvesPanel(panel)
  ...
  local function RefreshList()
    ...
    for si, cfg in ipairs(db.shelves) do
      ...
      -- Label
      local badge = (cfg.type == "bar") and "|cff88ccff[B" .. (cfg.barID or "?") .. "]|r " or "|cffcccc88[C]|r "
  ...
  local function RefreshDetail()
    ...
    if #db.docks > 1 then
      ...
      _, y = CreateDropdownButton(detailChild, indent, y, "Dock", dockOpts, curDockName, function(v)
        cfg.dockID = v
        DebouncedRebuild()
      end)
    end
    ...
    _, y = CreateSlider(detailChild, indent, y, "# of Rows", 1, isBar and 12 or 24, 1, cfg.numRows or 1, function(v)
      cfg.numRows = v
      DebouncedRebuild()
    end)

    _, y = CreateSlider(detailChild, indent, y, "Icon Size", 20, 56, 2, cfg.buttonSize or 36, function(v)
      cfg.buttonSize = v
      DebouncedRebuild()
    end)

    _, y = CreateSlider(detailChild, indent, y, "Icon Padding", 0, 12, 1, cfg.buttonPadding or 2, function(v)
      cfg.buttonPadding = v
      DebouncedRebuild()
    end)
    ...
    _, y = CreateSlider(detailChild, indent, y, "Handle Icon Size", 10, 32, 1, cfg.iconSize or 16, function(v)
      cfg.iconSize = v
      DebouncedRebuild()
    end)

    _, y = CreateSlider(detailChild, indent, y, "Handle Font Size", 8, 18, 1, cfg.labelFontSize or 12, function(v)
      cfg.labelFontSize = v
      DebouncedRebuild()
    end)
...
local function BuildGeneralPanel(panel)
  buildAceConfigPanel(panel, "Barshelf_General", "General", "Global behavior settings for all shelves and docks.")
end
...
local function BuildProfilesPanel(panel)
  ...
  buildAceConfigPanel(panel, "Barshelf_Profiles", "Profiles", "Manage saved variable profiles.")
end
...
local function RegisterAllPanels()
  ...
  mainPanel:SetScript("OnShow", function(self)
    ...
    desc:SetJustifyH("LEFT")
    desc:SetText(
      "Access your hidden action bars through floating popup shelves.\n\n"
